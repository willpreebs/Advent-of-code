...
// challenge: actually parse input instead of just hardcoding the stacks

pub fn find_tops_of_stacks(filename : &str) -> String {
    ...
    for line in reader.lines() {
        ...
        while num_to_move > 0 {
            ...
            let popped_crate = reversed_stack.get_mut(src_stack_index - 1).unwrap().pop().unwrap();
            reversed_stack[dst_stack_index - 1].push(popped_crate);
